...
class Registry:
    ...
    def _check_objects(self) -> list[Issue]:
        ...
        for obj in self._objects.values():
            ...
            if obj.pk_field not in fields:
                issues.append(Issue(severity="error", code="OBJECT_PK_MISSING",
                                    message=f"{obj.name}: 主键字段 {obj.pk_field} 不存在"))
            title = obj.title_field or obj.pk_field
            if title not in fields:
                issues.append(Issue(severity="error", code="OBJECT_TITLE_MISSING",
                                    message=f"{obj.name}: Title 字段 {title} 不存在"))
            if not _API_NAME_RE.match(obj.api_name):
                issues.append(Issue(severity="error", code="OBJECT_API_NAME_INVALID",
                                    message=f"{obj.name}: api_name={obj.api_name} 非法"))
            if not obj.source_table:
                issues.append(Issue(severity="error", code="OBJECT_NO_SOURCE_TABLE",
                                    message=f"{obj.name}: 缺少源系统承载表"))
            for fname in fields:
                if field_ownership(obj.model, fname) not in (OWN_SOURCE, OWN_ONTOLOGY, OWN_DERIVED):
                    issues.append(Issue(severity="error", code="FIELD_MISSING_OWNERSHIP",
    ...
    def _check_links(self) -> list[Issue]:
        ...
        for link in self._links:
            ...
            if link.target_type not in self._objects:
                issues.append(Issue(severity="error", code="LINK_UNKNOWN_TARGET",
                                    message=f"{link.name}: 目标类型 {link.target_type} 未注册"))
                continue
            if link.source_type == link.target_type:
                issues.append(Issue(severity="error", code="LINK_SELF_LOOP",
                                    message=f"{link.name}: 不允许自环链接"))
            # 外键所在侧：N:1 → source；1:N → target
            fk_model = (self._objects[link.source_type].model if link.cardinality == "N:1"
                        else self._objects[link.target_type].model)
            if link.fk_field not in fk_model.model_fields:
                issues.append(Issue(severity="error", code="LINK_FK_MISSING",
            ...
            if not link.inverse_name.startswith(prefix):
                issues.append(Issue(severity="error", code="LINK_INVERSE_MISMATCH",
    ...
    def _check_actions(self) -> list[Issue]:
        ...
        for action in self._actions.values():
            ...
            if not (eff.source_backed or eff.ontology_owned):
                issues.append(Issue(severity="error", code="ACTION_NO_EFFECTS",
    ...
    @staticmethod
    def _check_effect_field(issues: list[Issue], action: ActionDef, field: str,
                            expected_own: str, field_own: dict[str, str]) -> None:
        if field not in field_own:
            issues.append(Issue(severity="error", code="ACTION_EFFECT_UNKNOWN_FIELD",
                                message=f"{action.name}: 效果字段 {field} 不存在于任何对象"))
        elif field_own[field] != expected_own:
            issues.append(Issue(severity="error", code="ACTION_EFFECT_OWNERSHIP_MISMATCH",
