...
class WarningConcentration(BaseModel):
    ...
    top500_customer_id: str = own(
        OWN_SOURCE,
        "前500大客户风险结果表ID（FK→Top500CustomerRisk.top500_customer_id）",
    )
    ...
    group_customer_no: str = own(
        OWN_SOURCE, "集团客户编号（FK→GroupCustomer.group_customer_no）（脱敏）"
    )
    ...
    warn_rule: str = own(
        OWN_SOURCE, "集中度风险预警等级 字典项P082 红色预警B1和B2、黄色预警A1和A2"
    )
...
class WarningDerive(BaseModel):
    ...
    data_date: date = own(OWN_SOURCE, "数据日期")
    customer_id: str = own(
        OWN_SOURCE, "客户编号（FK→RiskCustomer.customer_id）（脱敏）"
    )
    customer_name: str = own(OWN_SOURCE, "客户名称（脱敏）")
    org_id: str = own(OWN_SOURCE, "机构编码")
    ...
    approve_order_status: Literal["PROCESS", "APPROVED", "REJECTED"] = own(
        OWN_SOURCE, "字典项P061 PROCESS-审批中 APPROVED-已通过 REJECTED-已驳回"
    )
    approve_order_id: str = own(
        OWN_SOURCE, "审批单ID（FK→ApproveOrder.approve_order_id）"
    )
...
class WarningDeviation(BaseModel):
    ...
    warn_rule: str = own(
        OWN_SOURCE, "背离预警触发规则标识 字典项P080 环比A、单月增幅阈值B、较年初C"
    )
    ...
    approve_order_id: str = own(
        OWN_SOURCE, "申请单ID（FK→ApproveOrder.approve_order_id）"
    )
...
class ApproveTodo(BaseModel):
    ...
    approve_task_id: str = own(
        OWN_SOURCE, "关联审批任务ID（岗位级，FK→ApproveTask.approve_task_id）"
    )
    user_id: str = own(OWN_SOURCE, "待办用户ID")
    approve_todo_status: Literal["UNHANDLED", "HANDLED", "INVALID"] = own(
        OWN_SOURCE,
        "待办状态 字典项P065：UNHANDLED-未处理 HANDLED-已处理 INVALID-已失效",
    )
...
class ApproveOperLog(BaseModel):
    ...
    approve_task_id: str = own(
        OWN_SOURCE, "审批任务ID（FK→ApproveTask.approve_task_id）"
    )
    operator_user_id: str = own(OWN_SOURCE, "操作人ID（脱敏）")
    operate_type: Literal["SUBMIT", "APPROVE", "REJECT"] = own(
        OWN_SOURCE, "操作类型：SUBMIT-提交 APPROVE-审批通过 REJECT-审批驳回"
    )
