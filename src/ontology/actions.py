...
CANONICAL_ERROR_CODES: tuple[str, ...] = (
    ...
    "REFUND_NOT_PENDING", "AMOUNT_EXCEEDS_PAID", "REFUND_NOT_ALLOWED",
)
...
class StateEffects(BaseModel):
    ...
    source_backed: list[str] = Field(default_factory=list, description="写回源系统的字段")
    ontology_owned: list[str] = Field(default_factory=list, description="本体自有状态字段")
    derived: list[str] = Field(default_factory=list, description="计算态（动作不写，仅标注）")
...
# ---- 参数模型（LLM 输出视为不可信输入：类型/枚举/边界经 Pydantic 校验） ----
...
ACTIONS: list[ActionDef] = [
    ActionDef(
        name="create_order",
        description=("下单：按客户与商品行创建订单（pending）并锁定库存 reserved+=qty；"
                     "目标仓可用量不足时拒绝（返回当前可用量）。"),
    ...
    ActionDef(
        ...
        preconditions=[
            ...
            Precondition(error_code="ORDER_NOT_CONFIRMABLE", summary="status 必须为 pending"),
        ],
    ...
    ActionDef(
        name="cancel_order",
        description=("取消订单（三问测试对象）：仅 pending/confirmed 且无 shipped/delivered "
                     "shipment 的订单可取消；取消后释放未发货行库存 reserved-=qty，"
                     "cancel_reason 落本体自有状态。已发货订单请走退款流程。"),
        params_model=CancelOrderParams,
        preconditions=[
            Precondition(error_code="ORDER_NOT_FOUND", summary="订单存在"),
            Precondition(error_code="ORDER_NOT_CANCELLABLE",
                         summary="status ∈ {pending, confirmed}（shipped/delivered/cancelled/refunded 拒绝）"),
            Precondition(error_code="SHIPPED_ORDER_CANNOT_BE_CANCELLED",
                         summary="无任何关联 shipment 处于 shipped/delivered（已发货拦截，三问 3）"),
        ],
        state_effects=StateEffects(source_backed=["Order.status", "Inventory.reserved_qty"],
                                   ontology_owned=["Order.cancel_reason"]),
        error_codes=["INVALID_PARAMS", "ORDER_NOT_FOUND", "ORDER_NOT_CANCELLABLE",
                     "SHIPPED_ORDER_CANNOT_BE_CANCELLED"],
    ),
    ActionDef(
        name="create_shipment",
        description=("发货：仅 confirmed 订单可发货；校验该仓物理在库 on_hand ≥ 订单行总量；"
                     "成功后建 Shipment(shipped)、订单 → shipped、on_hand-=qty、reserved-=qty。"),
        params_model=CreateShipmentParams,
        preconditions=[
            Precondition(error_code="ORDER_NOT_FOUND", summary="订单存在"),
            Precondition(error_code="ORDER_NOT_SHIPPABLE",
                         summary="order status=confirmed 且 warehouse 存在（后者归入参数校验）"),
            Precondition(error_code="INSUFFICIENT_INVENTORY",
                         summary="发货仓该品 on_hand_qty ≥ 订单行总量（物理在库校验，corner ④）"),
        ],
        state_effects=StateEffects(source_backed=["Shipment.status", "Order.status",
                                                  "Inventory.on_hand_qty", "Inventory.reserved_qty"]),
        error_codes=["INVALID_PARAMS", "ORDER_NOT_FOUND", "ORDER_NOT_SHIPPABLE",
                     "INSUFFICIENT_INVENTORY"],
    ),
    ActionDef(
        name="adjust_inventory",
        description=("人工盘点/纠错：设置 on_hand_qty = new_on_hand_qty；不能把已锁库存调没"
        ...
        preconditions=[
            ...
            Precondition(error_code="INSUFFICIENT_RESERVED",
    ...
    ActionDef(
        name="approve_refund",
        description=("审核退款（高风险，LLM 提议 + 用户确认双签）：仅 pending 可审；"
        ...
        preconditions=[
            Precondition(error_code="REFUND_NOT_PENDING",
                         summary="refund 存在且 status=pending（已审核再审 → 冲突，corner ③）"),
            Precondition(error_code="AMOUNT_EXCEEDS_PAID",
                         summary="amount_cents ≤ order.paid_cents − Σ(该单已批准退款)，否则超付（corner ③）"),
            Precondition(error_code="REFUND_NOT_ALLOWED",
                         summary="order 状态须为 shipped/delivered（已履约才可退），或 cancelled 且未退款"),
        ],
        state_effects=StateEffects(source_backed=["Refund.status", "Order.status"],
                                   ontology_owned=["Refund.review_note"]),
        error_codes=["INVALID_PARAMS", "REFUND_NOT_PENDING", "AMOUNT_EXCEEDS_PAID",
                     "REFUND_NOT_ALLOWED"],
        high_risk=True,
    ),
]
