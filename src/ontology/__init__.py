...
build_registry() 为统一入口：8 对象 / 8 链接 / 6 动作 + DES 6 对象（Material/Code +
P2 ChatBI 主体对象 ErpCustomer/Vendor/InventoryLocation/FinanceEntry，2026-08-21
Jack 拍板注册，2026-08-22 拍板 ErpCustomer 独立注册（源表 erp.KNA1，解决「Customer
同名冲突」，S1 零售 Customer 保留不动））/ 1 链接（hasCode）注册后返回 Registry，供
...
def build_registry() -> Registry:
    """注册全部对象/链接/动作（S1 8 + DES 6）与 DES self_check 钩子，返回 Registry。
