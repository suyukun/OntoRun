...
build_registry() 为统一入口：8 对象 / 8 链接 / 6 动作 + DES 2 对象（Material/Code）/ 1 链接
（hasCode）注册后返回 Registry，供 runtime（B1/B2）、API（B3）、Agent 工具生成（A2）消费。
...
def build_registry() -> Registry:
    """注册全部对象/链接/动作（S1 8 + DES 2）与 DES self_check 钩子，返回 Registry。
