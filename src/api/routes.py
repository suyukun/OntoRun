...
def _error_response(
    request_id: str, code: str, detail: Any = None, extra_outcome: str = "error"
) -> JSONResponse:
...
def get_actions(request: Request) -> dict:
    rt = _runtime(request)
    return _envelope(
        _new_request_id(), "ok", data=[_action_meta(a) for a in rt.registry.actions()]
    )
...
def list_objects(
    ...
    try:
        items, total = rt.query.list_objects(
            type, filters=filters, page=page, page_size=page_size
        )
    except (UnknownObjectType, UnknownFilterField) as exc:
        return _error_response(
            request_id,
            "OBJECT_TYPE_NOT_FOUND"
            if isinstance(exc, UnknownObjectType)
            else "UNKNOWN_FILTER_FIELD",
...
def object_detail(type: str, pk: str, request: Request) -> JSONResponse:
    ...
    try:
        ...
        return _error_response(
            request_id, "OBJECT_TYPE_NOT_FOUND", {"type": exc.type_name}
        )
    except ObjectNotFound as exc:
        return _error_response(
            request_id, "OBJECT_NOT_FOUND", {"type": exc.type_name, "pk": exc.pk}
        )
...
def link_traversal(
    type: str, pk: str, link_name: str, request: Request, direction: str = "out"
) -> JSONResponse:
    rt = _runtime(request)
    request_id = _new_request_id()
    try:
        ...
        return _error_response(
            request_id, "INVALID_DIRECTION", {"direction": exc.direction}
        )
    return JSONResponse(
        content=_envelope(
            request_id,
...
async def submit_action(action_name: str, request: Request) -> JSONResponse:
    ...
    # 审计完整性（red-team 重要 1）：X-Actor 白名单校验，非法值 400 拒绝。
    # 透传客户端可控 actor 会让源库已提交而 audit_log 落库失败（CHECK 约束），
    ...
    result: ActionResult = rt.engine.execute(
        action_name,
        params,
        actor=actor,
...
def audit_get(audit_id: str, request: Request) -> JSONResponse:
    ...
    if record is None:
        return _error_response(
            _new_request_id(), "OBJECT_NOT_FOUND", {"audit_id": audit_id}
        )
