...
def _warn_reason(level: str, level2: str, inputs: dict[str, float]) -> str:
    ...
    if level2 == "集中度超限":
        # F2 修复：RNG 浓度类信号的事由剥离百分比（不写死「敞口超限 10%」——该百分比是
        # RNG 独立流生成的，与源库集中度实算（concentration_limit 归集）脱钩）。
        # F8 修复：连「集团集中度敞口超限」硬断言也剥离——RNG 背景信号未绑定真实台账聚合，
        # 低集中度集团会被误判「敞口超限」；改监测语气「指标异动」，口径以实算为准。
        return f"集团集中度指标异动，触发{cn}预警"
    if level2 == "关联交易":
