...
# ---------------------------------------------------------------------------
ORG_POOL = (
    "安平集团总部", "安平银行", "安平证券", "安平信托", "安平保险",
    "安平租赁", "安平保理", "安平期货", "安平资产管理", "安平消费金融",
)
CUST_NAME_POOL = (
    "宏远实业", "泰和能源", "中科智造", "恒达地产", "远航物流", "金诚贸易",
    "蓝海科技", "华信建设", "鼎盛矿业", "瑞丰食品", "嘉源化工", "正大纺织",
    "盛世文旅", "东方商贸", "康德医药", "裕隆装备", "翔宇电子", "联发建材",
    "汇通汽贸", "天晟农业",
)
CUST_NAME_SUFFIX = ("有限公司", "集团", "股份公司", "有限责任公司", "控股集团")
GROUP_SUFFIX = ("集团有限公司", "控股集团有限公司", "产业发展集团", "实业集团")
FAMILY_NAMES = ("王", "李", "张", "刘", "陈", "杨", "黄", "赵", "周", "吴", "徐", "孙", "马", "朱")
GIVEN_NAMES = ("伟", "芳", "娜", "敏", "静", "磊", "军", "洋", "勇", "艳", "杰", "涛", "明", "超", "霞", "平", "刚", "华")
ZONE_POOL = (
    ("310000", "上海市"), ("330100", "杭州市"), ("320100", "南京市"), ("110000", "北京市"),
    ("440300", "深圳市"), ("440100", "广州市"), ("510100", "成都市"), ("370200", "青岛市"),
    ("210100", "沈阳市"), ("120000", "天津市"), ("420100", "武汉市"), ("500000", "重庆市"),
)
INDUSTRY_POOL = (
    ("C", "制造业"), ("D", "电力热力燃气及水生产和供应业"), ("E", "建筑业"),
    ("F", "批发和零售业"), ("G", "交通运输仓储和邮政业"), ("I", "信息传输软件和信息技术服务业"),
...
CUST_QUALITY_DIST = (("1", "正常", 0.85), ("2", "关注", 0.10), ("3", "次级", 0.03), ("4", "可疑", 0.015), ("5", "损失", 0.005))
# 预警等级分布（事实表整体：蓝为主）
WARN_LEVEL_DIST = (("BLUE", 0.70), ("YELLOW", 0.25), ("RED", 0.05))
# 五级分类分布（风险项目：明显偏险）
FIVE_CLASS_DIST = (("NORMAL", 0.55), ("ATTENTION", 0.28), ("SECONDARY", 0.11), ("DOUBTFUL", 0.045), ("LOSS", 0.015))
# 预警一级/二级主题（规则 5）
...
WARN_SOURCE_POOL = ("行内监测", "征信系统", "工商信息", "司法涉诉", "媒体报道", "现场检查", "模型预警")
...
# 处置措施类型（规则 6）
DEAL_TYPE_POOL = (
    "SUBMIT_REVIEW", "MGMT_REVIEW", "SUBMIT_APPROVE", "DEPT_LEADER_APPROVE",
    "WARN_REASON_UPDATE", "PUSH_SUBSIDIARY", "PUSH_HOLDING_LEADER", "LEADER_OPINION",
)
...
METRIC_DEFS = (
    ("M001", "集团对外融资余额", "亿元"), ("M002", "集中度敞口占比", "%"),
    ("M003", "逾期贷款余额", "亿元"), ("M004", "不良贷款率", "%"),
    ("M005", "拨备覆盖率", "%"), ("M006", "风险暴露额", "亿元"),
    ("M007", "押品估值覆盖率", "%"), ("M008", "预警信号数", "个"),
)
...
def warn_level_decide(
    ...
    if collateral_depreciation >= 0.30 or concentration_overrun >= 0.20 or risk_score >= 80:
        return "RED"
    if (
...
def concentration_calc(exposure: float, net_capital: float) -> dict[str, Any]:
    ...
    if ratio > CONCENTRATION_ORANGE:
        return {"ratio": ratio, "status": "RED_ALERT", "warn_level": "RED", "need_approval": True}
    if ratio > CONCENTRATION_WARN:
        return {"ratio": ratio, "status": "ORANGE_ALERT", "warn_level": "ORANGE", "need_approval": True}
    if ratio > CONCENTRATION_NORMAL:
        return {"ratio": ratio, "status": "YELLOW_ALERT", "warn_level": "YELLOW", "need_approval": False}
    return {"ratio": ratio, "status": "NORMAL", "warn_level": "NONE", "need_approval": False}
...
def collateral_val(rng: random.Random, base_value: float) -> dict[str, float]:
    ...
    return {"collateral_value": round(base_value * (1 - dep), 2), "depreciation_rate": dep}
...
def approve_flow(rng: random.Random, order_status: str, k: int = 3) -> list[tuple[str, str | None]]:
    ...
    if order_status == "REJECTED":
        return [("COMPLETED", "APPROVED") for _ in range(k - 1)] + [("COMPLETED", "REJECTED")]
...
def _asset_quality(rng: random.Random) -> tuple[str, str]:
    ...
    item = rng.choices(CUST_QUALITY_DIST, weights=[c[2] for c in CUST_QUALITY_DIST], k=1)[0]
...
def _distribute_counts(rng: random.Random, n: int, total: int, lo: int, hi: int) -> list[int]:
...
def _shareholder_block(rng: random.Random, i: int) -> dict[str, Any]:
    """第 i 大股东字段块（姓名 + 出资比例）。"""
    return {f"shareholder_name_{i}": _person_name(rng), f"shareholder_ratio_{i}": round(rng.uniform(0, 60), 2)}
...
def _warn_inputs_for_level(rng: random.Random, level: str, driver: str) -> dict[str, float]:
    ...
    return {"risk_score": score, "collateral_depreciation": coll, "concentration_overrun": conc, "related_change": rel}
...
def _five_class_inputs(rng: random.Random, category: str) -> dict[str, Any]:
    ...
    if category == "LOSS":
        return {"overdue_days": rng.randint(361, 720), "impairment_ratio": round(rng.uniform(0.5, 0.95), 4)}
    if category == "DOUBTFUL":
        return {"overdue_days": rng.randint(181, 360), "impairment_ratio": round(rng.uniform(0.3, 0.6), 4)}
    if category == "SECONDARY":
        return {"overdue_days": rng.randint(91, 180), "impairment_ratio": round(rng.uniform(0.15, 0.4), 4)}
    if category == "ATTENTION":
        return {"overdue_days": rng.randint(1, 90), "impairment_ratio": round(rng.uniform(0.01, 0.1), 4)}
    return {"overdue_days": 0, "impairment_ratio": round(rng.uniform(0.0, 0.005), 4)}


CONCENTRATION_RATIO_DIST = (("normal", 0.60), ("yellow", 0.25), ("orange", 0.10), ("red", 0.05))
