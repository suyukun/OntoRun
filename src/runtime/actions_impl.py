...
from src.runtime.action_engine import ActionHandler, Effect, Snapshot, Violation, Writeback, _now
...
class CreateOrderHandler(ActionHandler):
    def load_snapshot(self, snapshot: Snapshot, params: Any) -> dict:
        return {
            ...
            "items": [
                {"input": item, "product": snapshot.get("Product", item.product_id),
    ...
    def check(self, code: str, snapshot: dict, params: Any) -> tuple[bool, dict | None]:
        ...
        if code == "PRODUCT_NOT_FOUND":
            missing = [i["input"].product_id for i in snapshot["items"] if i["product"] is None]
            return not missing, {"missing_product_ids": missing} if missing else None
        if code == "PRODUCT_INACTIVE":
            inactive = [i["input"].product_id for i in snapshot["items"]
                        if i["product"] and i["product"]["status"] != "active"]
            return not inactive, {"inactive_product_ids": inactive} if inactive else None
        if code == "OUT_OF_STOCK":
            for item in snapshot["items"]:
                inv = item["inventory"]
                available = (inv["on_hand_qty"] - inv["reserved_qty"]) if inv else 0
                if available < item["input"].qty:
                    return False, {"product_id": item["input"].product_id,
    ...
    def compute_effects(self, conn: Any, snapshot: dict, params: Any
        ...
        now = _now()
        total = sum(i["input"].qty * i["product"]["price_cents"] for i in snapshot["items"])
        effects: list[Effect] = [Effect(object_type="Order", pk=order_id, prop="status",
                                        old=None, new="pending", note="创建订单")]
        writebacks: list[Writeback] = [Writeback(
            ...
            params=[order_id, params.customer_id, "pending", total, 0, "unpaid", "",
                    now, now], table="orders")]
        for j, item in enumerate(snapshot["items"]):
            ...
            writebacks.append(Writeback(
                ...
                params=[oi_id, order_id, item["input"].product_id, item["input"].qty,
                        item["product"]["price_cents"]], table="order_items"))
            inv_id = f"{MAIN_WAREHOUSE_ID}|{item['input'].product_id}"
            effects.append(Effect(object_type="Inventory", pk=inv_id, prop="reserved_qty",
            ...
            writebacks.append(Writeback(
                sql="UPDATE inventory SET reserved_qty = reserved_qty + ?, updated_at = ? "
                    "WHERE inventory_id = ?",
                params=[item["input"].qty, now, inv_id], table="inventory"))
...
class ConfirmOrderHandler(ActionHandler):
    ...
    def check(self, code: str, snapshot: dict, params: Any) -> tuple[bool, dict | None]:
        ...
        if code == "ORDER_NOT_CONFIRMABLE":
            ...
            return order is not None and order["status"] == "pending",                 {"order_status": order["status"]} if order else None
        return True, None

    def compute_effects(self, conn: Any, snapshot: dict, params: Any
                        ) -> tuple[list[Effect], list[Writeback]]:
        order = snapshot["order"]
        effects = [Effect(object_type="Order", pk=order["order_id"], prop="status",
                          old=order["status"], new="confirmed")]
        writebacks = [Writeback(
            sql="UPDATE orders SET status='confirmed', updated_at=? WHERE order_id=?",
            params=[_now(), order["order_id"]], table="orders")]
        return effects, writebacks
...
class CancelOrderHandler(ActionHandler):
    def load_snapshot(self, snapshot: Snapshot, params: Any) -> dict:
        ...
        items = snapshot.list_where("OrderItem", "order_id", params.order_id) if order else []
        shipments = snapshot.list_where("Shipment", "order_id", params.order_id) if order else []
        return {
            "order": order, "items": items, "shipments": shipments,
            "inventory": [
                snapshot.one("SELECT * FROM inventory WHERE warehouse_id=? AND product_id=?",
                             (MAIN_WAREHOUSE_ID, it["product_id"])) for it in items
            ],
        }
    ...
    def check(self, code: str, snapshot: dict, params: Any) -> tuple[bool, dict | None]:
        ...
        if code == "SHIPPED_ORDER_CANNOT_BE_CANCELLED":
            ...
            shipped = [s["shipment_id"] for s in snapshot["shipments"]
    ...
    def compute_effects(self, conn: Any, snapshot: dict, params: Any
        ...
        order = snapshot["order"]
        now = _now()
        effects: list[Effect] = [Effect(object_type="Order", pk=order["order_id"],
                                        prop="status", old=order["status"], new="cancelled")]
        if params.reason:
            effects.append(Effect(object_type="Order", pk=order["order_id"],
                                  prop="cancel_reason", old=None, new=params.reason,
                                  note="本体自有状态"))
        writebacks: list[Writeback] = [Writeback(
            sql="UPDATE orders SET status='cancelled', updated_at=? WHERE order_id=?",
            params=[now, order["order_id"]], table="orders")]
        for item, inv in zip(snapshot["items"], snapshot["inventory"]):
            ...
            writebacks.append(Writeback(
                sql="UPDATE inventory SET reserved_qty = reserved_qty - ?, updated_at = ? "
                    "WHERE inventory_id = ?",
                params=[item["qty"], now, inv_id], table="inventory"))
        return effects, writebacks
...
class CreateShipmentHandler(ActionHandler):
    def load_snapshot(self, snapshot: Snapshot, params: Any) -> dict:
        ...
        return {
            ...
            "inventory": [
                snapshot.one("SELECT * FROM inventory WHERE warehouse_id=? AND product_id=?",
                             (params.warehouse_id, it["product_id"])) for it in items
            ],
        }

    def validate_semantics(self, snapshot: dict, params: Any) -> Violation | None:
        if snapshot["warehouse"] is None:
            return Violation(error_code="INVALID_PARAMS", message="发货仓不存在",
    ...
    def check(self, code: str, snapshot: dict, params: Any) -> tuple[bool, dict | None]:
        ...
        if code == "INSUFFICIENT_INVENTORY":
            for item, inv in zip(snapshot["items"], snapshot["inventory"]):
                ...
                if on_hand < item["qty"]:
                    return False, {"product_id": item["product_id"], "on_hand_qty": on_hand,
    ...
    def compute_effects(self, conn: Any, snapshot: dict, params: Any
        ...
        now = _now()
        effects: list[Effect] = [
            Effect(object_type="Shipment", pk=shipment_id, prop="status",
                   old=None, new="shipped", note="创建发货单"),
            Effect(object_type="Order", pk=order["order_id"], prop="status",
                   old=order["status"], new="shipped"),
        ]
        writebacks: list[Writeback] = [
            Writeback(sql="INSERT INTO shipments (shipment_id, order_id, warehouse_id, status, "
                          "tracking_no, shipped_at) VALUES (?,?,?,?,?,?)",
                      params=[shipment_id, order["order_id"], params.warehouse_id, "shipped",
                              f"SF{seq:08d}", now], table="shipments"),
            Writeback(sql="UPDATE orders SET status='shipped', updated_at=? WHERE order_id=?",
                      params=[now, order["order_id"]], table="orders"),
        ]
        for item, inv in zip(snapshot["items"], snapshot["inventory"]):
            inv_id = f"{params.warehouse_id}|{item['product_id']}"
            effects.append(Effect(object_type="Inventory", pk=inv_id, prop="on_hand_qty",
                                  old=inv["on_hand_qty"], new=inv["on_hand_qty"] - item["qty"],
                                  note=f"出库 {item['qty']} 件"))
            effects.append(Effect(object_type="Inventory", pk=inv_id, prop="reserved_qty",
                                  old=inv["reserved_qty"], new=inv["reserved_qty"] - item["qty"]))
            writebacks.append(Writeback(
                sql="UPDATE inventory SET on_hand_qty = on_hand_qty - ?, "
                    "reserved_qty = reserved_qty - ?, updated_at = ? WHERE inventory_id = ?",
                params=[item["qty"], item["qty"], now, inv_id], table="inventory"))
        return effects, writebacks
...
class AdjustInventoryHandler(ActionHandler):
    ...
    def compute_effects(self, conn: Any, snapshot: dict, params: Any
        ...
        inv = snapshot["inventory"]
        effects = [Effect(object_type="Inventory", pk=inv["inventory_id"],
                          prop="on_hand_qty", old=inv["on_hand_qty"],
                          new=params.new_on_hand_qty)]
        writebacks = [Writeback(
            sql="UPDATE inventory SET on_hand_qty = ?, updated_at = ? WHERE inventory_id = ?",
            params=[params.new_on_hand_qty, _now(), inv["inventory_id"]], table="inventory")]
        return effects, writebacks
...
class ApproveRefundHandler(ActionHandler):
    def load_snapshot(self, snapshot: Snapshot, params: Any) -> dict:
        ...
        if order:
            ...
            approved_sum = sum(r["amount_cents"] for r in approved
    ...
    def check(self, code: str, snapshot: dict, params: Any) -> tuple[bool, dict | None]:
        ...
        if code == "AMOUNT_EXCEEDS_PAID":
            ...
            return ok, {"amount_cents": refund["amount_cents"], "paid_cents": order["paid_cents"],
                        "approved_sum": snapshot["approved_sum"], "limit": limit} if refund and order else None
    ...
    def compute_effects(self, conn: Any, snapshot: dict, params: Any
        ...
        now = _now()
        effects: list[Effect] = [
            Effect(object_type="Refund", pk=refund["refund_id"], prop="status",
                   old=refund["status"], new=params.decision),
            Effect(object_type="Refund", pk=refund["refund_id"], prop="reviewed_at",
                   old=refund.get("reviewed_at"), new=now),
            Effect(object_type="Refund", pk=refund["refund_id"], prop="review_note",
                   old=refund.get("review_note"), new=params.review_note, note="本体自有状态"),
        ]
        writebacks: list[Writeback] = [Writeback(
            sql="UPDATE refunds SET status=?, reviewed_at=? WHERE refund_id=?",
            params=[params.decision, now, refund["refund_id"]], table="refunds")]
        full_refund = (params.decision == "approved"
                       and refund["amount_cents"] == order["paid_cents"])
        if full_refund:
            effects.append(Effect(object_type="Order", pk=order["order_id"], prop="status",
                                  old=order["status"], new="refunded", note="整单退款"))
            writebacks.append(Writeback(
