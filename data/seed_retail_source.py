...
def generate_customers(rng: random.Random) -> list[tuple]:
    ...
    for i in range(1, N_CUSTOMERS + 1):
        segment = _weighted_choice(
            rng, [("retail", 0.7), ("sme", 0.2), ("corporate", 0.1)]
        )
        if segment == "retail":
...
def _order_item_spec(
    rng: random.Random, active_products: list[str]
) -> list[tuple[str, int]]:
...
def simulate(rng: random.Random, products: list[tuple], orders: list[tuple]) -> dict:
    ...
    for order_id, customer_id, status, created_at in orders:
        ...
        for j, (pid, qty) in enumerate(spec):
            item_rows.append(
                (f"OI-{order_id[4:]}-{j + 1}", order_id, pid, qty, product_prices[pid])
            )
        ...
        if status == "cancelled":
            ...
            ship_status = (
                "delivered" if status in ("delivered", "refunded") else "shipped"
            )
...
def _ship(
    ...
    inventory: dict[tuple[str, str], dict[str, int]],
) -> tuple[bool, dict]:
...
def generate_refunds(
    rng: random.Random,
    orders: list[tuple],
    shipped_order_ids: list[str],
    created_at_by_order: dict[str, datetime],
) -> list[tuple]:
    ...
    if len(shipped_paid) >= 3:
        ...
        rows.append(
            (
                f"REF-{counter:04d}",
                shipped_paid[0],
                int(paid_by_order[shipped_paid[0]] * 0.6),
                "pending",
                "商品与描述不符",
                reviewed(shipped_paid[0], 1),
                None,
            )
        )  # 双签演示
        counter += 1
        rows.append(
            (
                f"REF-{counter:04d}",
                shipped_paid[1],
                paid_by_order[shipped_paid[1]] + 5000,
                "pending",
                "重复扣款申请（超实付演示）",
                reviewed(shipped_paid[1], 1),
                None,
            )
        )
        counter += 1
        rows.append(
            (
                f"REF-{counter:04d}",
                shipped_paid[2],
                int(paid_by_order[shipped_paid[2]] * 0.4),
                "approved",
                "部分退货",
                reviewed(shipped_paid[2], 1),
                reviewed(shipped_paid[2], 2),
            )
        )
        ...
        if roll < 0.15:
            ...
            rows.append(
                (
                    f"REF-{counter:04d}",
                    oid,
                    int(paid_by_order[oid] * rng.uniform(0.3, 1.0)),
                    "pending",
                    rng.choice(REFUND_REASONS),
                    reviewed(oid, 1),
                    None,
            ...
            rows.append(
                (
                    f"REF-{counter:04d}",
                    oid,
                    int(paid_by_order[oid] * rng.uniform(0.3, 0.8)),
                    "rejected",
                    rng.choice(REFUND_REASONS),
                    reviewed(oid, 1),
                    reviewed(oid, 2),
...
def build_database(db_path: str | Path | None = None, seed: int = SEED) -> Path:
    ...
    shipped_order_ids = [
        o[0] for o in sim["orders"] if o[2] in ("shipped", "delivered")
    ]
    refunds = generate_refunds(
        rng, sim["orders"], shipped_order_ids, created_at_by_order
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="生成零售供应链源系统库（确定性 seed）"
    )
