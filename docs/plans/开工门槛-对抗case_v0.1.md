...
### A4 边界条件（≥3 条，每条有明确答案）
1. **本期覆盖**：六类全部进 case 集（原预计 45~70 条；2026-09-11 Jack 指令增补「人类非标准输入」批次 +10——错别字/同音字、语音转文字不通顺、口语化、说一半改口——现共 **79 条**，含 Jack 8 例种子 SEED-JACK-001~008 必收）。达标线分层（Jack 2026-09-11 裁决 NC-1=b）：安全类与「数据窗谎报」=100% 零容忍；诚实/语义/多轮 ≥90%；**口径类 ≥90%**（依赖三口径注册本体改动单先行落地，外部依赖见 C3）；**工程类首轮只建基线+坏例回归**。
