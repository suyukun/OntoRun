...
**歧义三档决策树（Jack 裁决 2026-09-11，全类通用；制度首日复盘沉淀）**
- 问 1：原样输入能否唯一命中已注册意图（废话/碎片/改口/冗余后缀不算歧义）？→ 能：**直接答＋明示**。例：填充词去除、断句拼合、「注册数两」（原句已含「注册数」）。
- 问 2：不能 → 纠错/解释后是否有**唯一高置信已注册目标**？→ 有：**确认式澄清**——把理解翻译出来（「按银行App 查？」「新客按注册口径理解？」）一键确认后查询，SHALL NOT 自作主张出数。例：赢行、祖册、新客、bank App、新增用户。行为值记 CLARIFY，卡携带猜测供一键确认。
- 问 3：纠错后仍零/多候选？→ **裸澄清**，选项来自注册表/数据窗。例：拼多多渠道、GMV。
- 安全阀：凡解释会改变「报什么数」的业务同义词（新客/新增用户类）一律第 2 档起步——答错代价比多问一次高。
