...
- 最小实现：只做要求的功能，不做投机性抽象/配置/容错；能写 50 行就不写 200 行。判据：问自己「高级工程师会认为这过度设计吗？」是，就简化。
- 只动任务要求的代码：不顺手"改进"相邻代码/注释/格式；不重构没坏的东西；风格跟随现有代码。发现无关死代码：提出来但不删（除非要求）。只清理自己改动造成的孤儿（无用 import/变量/函数）。
- 风格示例（本体对象定义）：
```python
...
## 开工门槛制度（2026-09-11 定，Jack 指令固化）
- 任何编码活派发前过 `docs/plans/开工门槛-模板.md`：S/M/L 分级；M/L 级填门槛稿，稿内存在 `NC-OPEN` 未清零禁止派编码活（派活前自查：`grep -c 'NC-OPEN' <门槛稿>` = 0）。
- 交付按模板收口五步（converge）：基线绿 + diff↔条目对照 + 演示路径走通 + NC 全清 + commit 引用门槛稿。
- 返工 ≥2 轮止损：停、修 spec、新会话只带 spec+失败清单，不许第 3 轮 patch。
- 存量门槛稿（UX/对抗case/管理台 v0.1）收口时按模板 §NC 补登状态标记。
...
## 阶段声明（识别与治理）
- 当前阶段：**S5（财富广场本体论落地·方法论真实场景验证）**——S1（零售 MVP 闭环）/ S2（发布期收尾）/ S3（金控风险演示版，tag v0.3.0，698 passed）已完成；S4（金控 Agent 演示）彩排九轮过线、M5 方案待签发后暂挂。2026-09-08 方向再锚：财富广场数据分析体系 = 方法论第一落点（价值先行三步：真问题+最小本体切片 → 手工接真数据出第一个答案 → 验证后固化），金控 = 方法论验证后的跨域第二落点（详见 docs/财富广场本体落地-分层架构与优先级_v0.1.md、docs/v0.4-核心链口径包-v0.3.md）。2026-09-09 展示层基线 tag v0.4.0（ChatBI 壳「财富ThoughtSpot」+ Agent 式思考流交互，ADR-0009；交互范式不回退）。**2026-09-10 S5 注册场景 T1a（无数据段）收官**（血缘 17/17 脚本·54 边·28 表·0 失败；镜像库 data/fortune_mirror.duckdb 22 表；数仓原始 ETL SQL 逐字重放零失真），T2 语义层交付（registry+compiler，commit 6a186f3）。**2026-09-11 当前主线 = 开工门槛制度落地 + 管理台重构（M1+M2 代码 100% 完成，待 Jack D3 演示）+ UX 优化（后续批 8 任务收口，对抗回归 59/79 PASS）**；语义层由 Rose 线交付（非 Jack 亲做）。2026-09-11 域框架改判：管理台登记口径 = 非 ADS 22 物理表（逻辑 20），金控 10 域框架（原 7 域提案被否，registry.py 已重登记 c5598c2；动 registry.py 前先读 docs/research/数据域划分调研-管理台域图_v0.1.md §9）。2026-09-11 LLM 路由切智谱 glm-5.3-flash（.env 变量名仍为 DEEPSEEK_*）。
- 治理缺口（2026-09-11 制度首日复盘自承，未闭环）：①门禁靠 Rose 自觉 grep，尚无 governance 测试（违反铁律②「规则必须能机器验证」）②converge 收口靠自觉；spec-kit 只调研未试用。详见 docs/制度首日复盘_2026-09-11.md；SPEC 机制最佳实践与工具选型讨论窗进行中。
- 技术债登记：docs/tech-debt.md（探索期欠的测试/CI 债显式记录，迁移时偿还）。
- 决策日志：关键决策记 docs/adr/（现状/备选/决定/理由/后果）；已积累的 ADR 雏形：补丁 v0.3.1、两档测试制度、开发哲学与阶段策略。

## LLM 调用与限流纪律
- 优先使用已付费套餐内模型（默认模型现值以 settings.yaml 的 agent-default-model 为准，2026-09-01 起为 qwen-api/qwen3.8-flash；默认配置易漂移——三天内火山→智谱→qwen 三变，派发前必须核对现值；火山 Coding Plan Pro 存在日间限流窗口（2026-09-01 实证），限流期用默认模型、解除后可作备用池）；自费 API（plan 外 provider）仅兜底，启用前告知 Jack。
- **子代理必须显式指定 provider/model**（workflow agent() 的 opts，或派发前核对 agent-default-model 配置）——默认配置切换会让派活绑错账户（2026-09-01 实证：子代理绑旧火山配置撞额度、报"额度不足"）。
- 火山账户级 429 的症状识别：子代理长时间 running 但零文件动静、零提交 = 30s→300s×4 的静默退避循环（约 20 分钟），先查限流窗口再判僵死；interrupt/接管消息要等其 turn 边界才被读取。
- 子代理并发 ≤4 个长任务（2026-09-10 Pro 会员更新，原 ≤2；长任务 = 预计 >10 分钟或多轮工具调用的编码/审查任务）；交付即收尾，不让 continuable 长挂（idle 占配额窗口）；**重派接管前必须终止旧会话并清点工作区，防同任务双飞写同一文件**。
...
6. **待定项未定不派活**：S1 零售 MVP 闭环已完成；S5 聚焦财富广场本体论落地（2026-09-08 方向再锚：第一落点=数据分析体系，金控降为验证后第二落点）。2026-09-11 起派活前额外过 AGENTS.md「开工门槛制度」节：M/L 级门槛稿 grep NC-OPEN = 0 方可派活，NC 与默认建议均须 Jack 事前确认；剩余待定 Jack 拍板前不派编码活。
