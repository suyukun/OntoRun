...
## 阶段声明（识别与治理）
- 当前阶段：**S4（金控 Agent 演示·处长验收）**——S1（零售 MVP 闭环）/ S2（发布期收尾）/ S3（金控风险演示版，tag v0.3.0，698 passed）已完成；S4 聚焦金控风险预警单场景，Agent 对话为唯一一级入口，按《核心链口径包 v0.3》再造数据/规则/交互；终极验收 = 中信金控风险合规部风险技术管理处处长 5 分钟内感受价值、显专业、看得出智能体优势（详见 docs/v0.4-方向重锚.md、docs/v0.4-核心链口径包-v0.3.md）。
...
6. **待定项未定不派活**：S1 零售 MVP 闭环已完成；S4 起聚焦金控风险预警单场景（方向重锚 v0.4，详见 docs/v0.4-方向重锚.md 与核心链口径包）；剩余待定 Jack 拍板前不派编码活。
