...
> 三条铁律（违反即停，详情见下文各节）：
> ① 第一版先跑通：探索期测试只锁核心不变量，子代理禁止跑全量 pytest；全量仅 Rose 阶段末跑。
> ② 规则必须能机器验证：软规则必须配套 governance 测试/CI 门禁，否则视为未生效。
> ③ 单一事实来源：git + AGENTS.md 阶段声明；关键决策记 ADR。
...
- **独立审查（风险导向，对应全局开发哲学）**：P4 的 E6 快照审计/写回回路 = 高风险变更，交付后即时审查一次；P6 收口 = 发布门禁，必做独立终审；其余不设固定审查。
- **CI 引入**：P6 前不建 CI（本地增量 + 阶段末全量已够）；P6 收口后建轻 CI（lint + 全量 pytest + vitest）。
- 测试失败修实现，不修测试（除非测试本身错）。

## 阶段声明（识别与治理）
- 当前阶段：**S3（金控风险预警演示）**——S1（零售 MVP 闭环）/ S2（发布期收尾）已完成（659 passed / CI 全绿）；S3 把语义接口+DES 能力落地到金控风险预警系统演示场景（详见 docs/S3-工作目标与计划-v1.0.md）。
- 技术债登记：docs/tech-debt.md（探索期欠的测试/CI 债显式记录，迁移时偿还）。
- 决策日志：关键决策记 docs/adr/（现状/备选/决定/理由/后果）；已积累的 ADR 雏形：补丁 v0.3.1、两档测试制度、开发哲学与阶段策略。
